...
class TopSimp:
    def __init__(self, nelx: int = 45, nely: int = 30, volfrac: float = 0.5, 
                penal: float = 3.0, rmin: float = 1.5):
        '''
        初始化拓扑优化问题

        Parameters:
        - nelx (int): 水平方向的单元数. Defaults to 45.
        - nely (int): 垂直方向的单元数. Defaults to 30.
        - volfrac (float): Volume fraction, 表示材料将占据的设计空间的期望分数. Defaults to 0.5.
        - penal (float): Penalization power, 在 SIMP 中控制中间密度的 penalization. Defaults to 3.0.
        - rmin (float): Filter 半径. Defaults to 1.5.
        '''
        self._nelx = nelx
        self._nely = nely
    ...
    def OC(self, volfrac, x, dc, passive):
        """
        ...
        Parameters:
            ...
            - x (ndarray): 密度分布矩阵 (nely, nelx).
            - dc (ndarray): 原始的灵敏度矩阵 (nely, nelx).
            - passive (ndarray): 一个区域，在自由变化的单元处为 0，在固定的单元处为 1 (nely, nelx)
