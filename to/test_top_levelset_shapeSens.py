...
class TopLevelSet:
    ...
    def visualize_level_set(self, phi, subplot_position):
        nely, nelx = phi.shape  # 获取 phi 的形状
        
        ax = plt.subplot(1, 2, subplot_position)  # 两个子图，此处的 2 是子图的总数
        
        # 使用得到的 nelx 和 nely 来设置轴的范围
        im = ax.imshow(phi, extent=[0, nelx, 0, nely], origin='lower', cmap='jet', vmin=phi.min(), vmax=phi.max())
        
        ax.contour(phi, levels=[0], colors='red', linewidths=2, extent=[0, nelx, 0, nely])
        
        ax.set_title(f"Iteration {subplot_position}")
        ax.set_xlabel('x')
        ax.set_ylabel('y')
        plt.colorbar(im, boundaries=np.linspace(phi.min(), phi.max(), 10), ax=ax)
    ...
    def optimize(self, Num: int = 2):
        ...
        plt.figure(figsize=(12, 6))  # 创建一个新的图形窗口，宽度为12，高度为6
