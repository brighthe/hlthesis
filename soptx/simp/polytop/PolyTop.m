...
function [z, V, fem] = PolyTop(fem, opt)
    Iter = 0; Tol = opt.Tol * (opt.zMax - opt.zMin); 
...
function [zNew, Change] = UpdateScheme(dfdz, g, dgdz, z0, opt)
    ...
    while l2-l1 > 1e-4
        ...
        if (g + dgdz'*(zNew-z0) > 0), l1 = lmid; else, l2 = lmid; end
    end
...
function [weight, point] = PolyQuad(nn)
    [W, Q] = TriQuad;                      % integration pnts & wghts for ref. triangle
