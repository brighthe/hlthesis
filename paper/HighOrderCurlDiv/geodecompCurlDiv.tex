...
\begin{document}
\title[High Order Edge and Face Elements]{Geometric Decomposition and Efficient
Implementation of High Order Face and Edge Elements}
\author{Chunyu Chen}%
\address{School of Mathematics and Computational Science, Xiangtan University; National Center of  Applied Mathematics in Hunan, Hunan Key Laboratory for Computation and Simulation in Science and Engineering Xiangtan 411105, China }%
...
\address{School of Mathematics and Computational Science, Xiangtan University; National Center of  Applied Mathematics in Hunan, Hunan Key Laboratory for Computation and Simulation in Science and Engineering Xiangtan 411105, China }%
 ...
 \thanks{The first and fourth authors were supported by the National Natural
Science Foundation of China (NSFC) (Grant No. 12371410, 12261131501), and the construction of innovative provinces in Hunan Province (Grant No. 2021GK1010). The second author was supported by NSF DMS-2012465 and DMS-2309785. The third author was supported by the National Natural Science Foundation of
...
This paper introduces node-based basis functions for high-order finite elements,
with a specific focus on Lagrange, BDM (Brezzi-Douglas-Marini)
\cite{BrezziDouglasMarini1985,Nedelec1986,BrezziDouglasDuranFortin1987}, and
second-kind N\'ed\'elec elements \cite{Nedelec1986,ArnoldFalkWinther2006}. These finite elements are subspaces of the function spaces $H^1$, $H(\div)$, and $H(\curl)$, respectively, with shape functions being $\mathbb P_k^n$, where $k$ signifies the polynomial degree and $n$ denotes the geometric dimension. It is worth noting that due to varying definitions of trace spaces, these finite elements exhibit distinct characteristics.
...
functions for BDM and second-kind N\'ed\'elec elements is a more intricate
endeavor. Traditional approaches involve the Piola transformation, where basis
...
the case of second-kind N\'ed\'elec elements) or contravariant (to maintain
normal continuity, for BDM elements) Piola transformation. Detailed explanations
of this approach can be found in
\cite{ervin2012computational,EfficientAssemblyOfHdivHcurl}, and implementation
is in open-source software such as MFEM~\cite{anderson2021mfem} and
Fenics~\cite{alnaes2015fenics}.

Arnold, Falk and Winther, in \cite{ArnoldFalkWinther2009}, introduced a
geometric decomposition of polynomial differential forms of various orders.
Basis functions based on Bernstein polynomials were proposed, paving the way for
subsequent advancements. In
\cite{ainsworth2015bernstein,ainsworth2018bernstein}, basis functions founded on
Bernstein polynomials were explored, accompanied by fast algorithms for matrix
assembly.  Additionally, hierarchical basis functions for $H(\curl)$-conforming
finite elements were introduced in
\cite{ainsworth2003hierarchic,xin2011well,xin2011construction,xin2013construction}.
\XH{Hierarchical geometric decomposition of $H(\div)$-conforming finite elements in two and three dimensions is discussed in \cite{DeSiqueiraDevlooGomes2010,DeSiqueiraDevlooGomes2013,CastroDevlooFariasGomesEtAl2016}, and hierarchical geometric decomposition of $H(\curl)$-conforming finite elements in two dimensions can also be found in \cite{DeSiqueiraDevlooGomes2010,DeSiqueiraDevlooGomes2013}.}
...
vertices. Nevertheless, nodal basis functions for BDM elements and second-kind N\'ed\'elec elements remained a challenge.

The key to constructing basis functions for BDM and second-kind N\'ed\'elec elements lies in ensuring trace continuity on the element's boundary. For BDM elements, this continuity is normal, while for second-kind N\'ed\'elec elements, it is tangential. We first elucidate the basis of Lagrange elements, viewing them as dual to degrees of freedom formed by values at interpolation points. This concept is readily extended to vector polynomial spaces, where each interpolation point defines a frame encompassing tangential and normal directions. This allows unique determination of vector polynomial functions, and continuity of either tangential or normal components leads to the degrees of freedom of BDM or second-kind N\'ed\'elec elements. The dual basis functions for these elements can be explicitly derived based on the basis functions of Lagrange elements.

%\LC{Mention the limitation: we only consider the second type not the first type.}

An essential aspect of practical finite element implementations is the
management of local Degrees of Freedom (DoFs), ensuring proper mapping to global
DoFs. This is critical for maintaining the correct continuity between elements
and correctly integrating matrices and vectors across the system. Remarkably,
there is limited literature addressing degree of freedom management for
high-order finite elements. In this paper, we delve into the issue of global
indexing for interpolation points in arbitrary-degree Lagrange finite
elements. We systematically discuss degree of freedom management for Lagrange,
BDM, and second-kind N\'ed\'elec elements, with the aim of simplifying the implementation of high-order finite element methods.
...
construction of basis functions for Lagrange, BDM, and second-kind N\'ed\'elec elements,
...
mixed Poisson problem and Maxwell problem using BDM and second-kind N\'ed\'elec elements, respectively, to validate the correctness of the proposed basis function construction method.

\section{Preliminaries}\label{sec:pre}
In this section, we provide essential foundations for our study. We introduce multi-indices, simplicial lattices, and interpolation points, which play a crucial role in our design of finite element methods. We explain sub-simplices and their relations. Furthermore, we introduce the concept of bubble polynomials. 
...
\begin{equation*}
R_n(\boldsymbol \alpha)=\sum_{i=1}^n{n+1-i\choose \alpha_i+\alpha_{i+1}+\cdots+\alpha_n+n-i}.
\end{equation*}
...
$$
which is called the set of interpolation points with degree $k$ on $T$; see Fig.~\ref{fig:interpointk4} for $k=4$ and $n=2$. Given $\bs \alpha\in \mathbb T^{n}_k$, the barycentric coordinate of $\bs \alpha$ is given by
$$
...
\begin{figure}[htp]
\label{fig:interpolationpoint}
\centering
\includegraphics[width=0.5\textwidth]{figures/tridof-4.pdf}
\caption{The interpolation points associated to $\mathbb T^2_4$ and their
    indices and corresponding multi-indexs. }
...
Elements of $\Delta_0(T) = \{{\bs x}_0, \ldots, {\bs x}_n\}$ are $n+1$ vertices of $T$ and $\Delta_n(T) = T$. The capital letter $F$ is reserved for an $(n-1)$-dimensional face of $T$ and $F_i\in\Delta_{n-1}(T)$ denotes the face opposite to ${\bs x}_i$ for $i=0,1,\ldots, n$.

%It is the combinatory structure of the simplex that plays an important role in the construction. 
For a sub-simplex $f\in \Delta_{\ell}(T)$, we will overload the notation $f$ for both the geometric simplex and the algebraic set of indices. Namely $f = \{f(0), \ldots, f(\ell)\}\subseteq \{0, 1, \ldots, n\}$ and 
$$
...
\end{equation}
For example, assume $f = \{ 1, 3, 4\}$, then for 
$\alpha = ( \alpha_{0},\alpha_{1},\alpha_{2})\in \mathbb T^{\ell}_k(f)$, the extension $ E(\alpha)= (0, \alpha_{0}, 0, \alpha_{1}, \alpha_{2}, \ldots, 0).$ With a slight abuse of notation, for a node $\alpha_f\in \mathbb T^{\ell}_k(f)$, we still use the same notation $\alpha_f\in \mathbb T^{n}_k(T)$ to denote such extension. 
The geometric embedding $\bs x_{E(\alpha)}\in f$ which justifies the notation $\mathbb T^{\ell}_k(f)$ and its geometric embedding will be denoted by $\mathcal X_f$, which consists of interpolation points on $f$.  
...
\end{equation}
Note that a lot of duplications exists in \eqref{eq:Xunion}. A direct sum of the interpolation set is given by 
...
%$\mathbb T^{\ell}_k(\mathring{f})$ is the set of lattice points whose geometric embedding is in the interior of $f$, i.e., $\mathring{\mathcal X}_f$. 
...
For the polynomial space $\mathbb P_k(T)$ with $k\geq 1$ on an $n$-dimensional simplex $T$, we have the following geometric decomposition of Lagrange element~\cite[(2.6)]{ArnoldFalkWinther2009} and a proof can be found in \cite{Chen;Huang:2021Geometric}. 
...
%The integral at a vertex is understood as the function value at that vertex and $\mathbb P_k({\bs x}) = \mathbb R$. A proof of the unisolvence can be found in~\cite{Chen;Huang:2021Geometric}.
...
$$
V_k^{\rm L}(\mathcal T_h) = \{ v\in C(\Omega): v|_{T}\in \mathbb P_k(T), T\in \mathcal T_h\},
$$
...
$$
where $|\Delta_{\ell}(\mathcal T_h)|$ is the cardinality of  number of $\Delta_{\ell}(\mathcal T_h)$, i.e., the number of $\ell$-dimensional simplices in $\mathcal T_h$.
...
The geometric decomposition can be naturally extended to vector Lagrange
...
\begin{itemize}
\item $\mathscr T^f := \spn \{ \bs t_1, \ldots, \bs t_{\ell} \}, $ is the tangential plane of $f$;

\item $\mathscr{N}^f :=  \spn \{ \bs n_1, \ldots, \bs n_{n - \ell} \} $ is the normal plane of $f$. 
\end{itemize}
...
%
When $\ell = 0$, i.e., for vertices, no tangential component, and for $\ell = n$, no normal component. We have the trivial decompositions
...
$$ 
which is a space of $\ell$-dimensional vectors on the tangential space with 
vanishing trace $\tr^{\grad}$ on $\partial f$. As a corollary of Theorem \ref{thm:Lagrangedec} and $t-n$ decomposition \eqref{eq:tndec}, we have the following decomposition of the vector Lagrange elements. 
...
Previously the DoFs are given by moments on sub-simplexes. Now we present a set of DoFs as function values on the interpolation points and give its dual basis for the $k$-th order Lagrange element on an $n$-simplex.

\begin{lemma}[Lagrange interpolation basis functions]\label{le:li}
A basis function of $k$-th order Lagrange finite element space on $T$ is:
$$
...
For an interpolation point $\bs x\in \mathcal X_{T}$, let $\{\bs e^i_{\bs x}, i=0,\ldots, n-1\}$ be a basis of $\mathbb R^n$, and its dual basis is denoted by $\{\hat{\bs e}^i_{\bs x}, i=0,\ldots, n-1\}$. When $\{\bs e^i_{\bs x}, i=0,\ldots, n-1\}$ is orthonormal, its dual basis is itself. 
...
\begin{lemma}\label{lem:basisfunctions}
    A polynomial function $\bs u \in \mathbb P_k^n(T)$ can be uniquely determined by the DoFs:
  $$
N^i_{\bs \alpha }(\bs u):=  \bs u(\bs x_{\bs \alpha}) \cdot \bs e_{\bs x_{\bs \alpha}}^i, \quad \bs x_{\bs \alpha} \in \mathcal X_{T}, i = 0,\ldots, n-1.
  $$
...
$$
N^j_{\bs \beta }(\bs{\phi}_{\bs \alpha}^i) =   \bs{\phi}_{\bs \alpha}^i(\bs x_{\beta}) \cdot \bs e_{\bs x_{\beta}}^j  =   \phi_{\bs \alpha}(\bs x_{\beta}) \hat{\bs e}_{\bs x_{\bs \alpha}}^i \cdot \bs e_{\bs x_{\beta}}^j  = \delta_{i}^j \delta_{\bs \alpha}^{\bs \beta},
$$ 
...
Given a triangulation $\mathcal T_h$ and a piecewise smooth function $\bs u$, it is well known that $\bs u\in H(\div, \Omega)$ if and only if $\bs n_F \cdot \bs u$ is continuous across all faces $F\in \Delta_{n-1}(\mathcal T_h)$, where $\bs n_F$ is a global normal vector of $F$. An $H(\div)$-conforming finite element is thus also called a face element.
...
\mathbb P_k^n(T) &= \Oplus_{F\in\Delta_{n-1}(T)} \left ( \mathbb P_k(F)\bs n_F \right )\oplus \mathbb B_k(\div, T).
\end{align}
\end{theorem}
\begin{proof}
...
$$
\Oplus_{\ell=1}^{n-1}\Oplus_{f\in\Delta_{\ell}(T)}\Oplus_{F\in \Delta_{n-1}(T), f\subseteq F} = \Oplus_{F\in \Delta_{n-1}(T)}\Oplus_{\ell=1}^{n-1}\Oplus_{f\in\Delta_{\ell}(F)},
$$
...
In decomposition \eqref{eq:divpolynomialdecomp}, we single out $\mathbb P_1^n(T)$ to emphasize a $\div$-conforming element can be obtained by adding $\div$-bubble and normal component on sub-simplexes starting from edges. In \eqref{eq:divpolynomialdecomp2}, we group all normal components facewisely which leads to the classical BDM element.
...
%%\LC{Explain local and global normal basis.}
%%\LC{include the element. Choose all normal components are global.}
...
\end{theorem}
\begin{proof}
The unisolvence of DoFs \eqref{eq:dofvecdiv} for $\mathbb P_k^n(T)$ follows from Corollary~\ref{lm:vecLdec} and switching $\{\bs n_1^f, \dots, \bs n_{n-\ell}^f\}$ to $\{\bs n_{F}: F\in \Delta_{n-1}(T), f\subseteq F\}$.
...
For the efficient implementation, we change the DoFs as the function values at interpolation nodes and combine with $t-n$ decomposition to present a nodal basis for the BDM element.  
 
Given an $f\in \Delta_{\ell}(T)$, we choose $\{\bs n_F, F\in \Delta_{n-1}( T), f\subseteq F\}$ as the basis for its normal plane $\mathcal N^f$ and an arbitrary basis $\{ \bs t_i^f, i=1,\ldots, \ell \}$ for the tangential plane. Denote by $\{\hat{\bs n}_F\in \mathcal N^f, F\in \Delta_{n-1}( T), f\subseteq F\}$ the basis of $\mathcal N^f$ dual to $\{\bs n_F , F\in \Delta_{n-1}( T), f\subseteq F\}$, i.e.,
$$
...
For $f\in \Delta_{\ell}(T)$ and $e\in\partial f$, let $\bs n_{f,e}$ be an unit normal vector of $e$ but tangential to $f$. When $\ell=1$,$f$ is an edge and $e$ is a vertex. Then $\bs n_{f,e}$ is the edge vector of $f$. When $\ell=n$, choose $\bs n_{f,e}$ as the normal vector $\bs n$.
Using these notations we can give an explicit expression of the dual basis $\{\hat{\bs n}_F\in \mathcal N^f, F\in \Delta_{n-1}( T), f\subseteq F\}$.
...
\end{equation}
where $f+i$ denotes the $(\ell+1)$-dimensional face in $\Delta_{\ell+1}(T)$ with vertices $f(0),\dots, f(\ell)$ and $i$ for $i\in f^*$.
\end{lemma}
\begin{proof}
...
For an interpolation point $\bs x\in \mathcal X_{T}$, we collect the $t-n$ basis together and write as $\{\bs e^i_{\bs x}, i=0,\ldots, n-1\}$. We can always choose an orthonormal basis for the tangential plane $\mathscr T^f$ but for the normal plane $\mathscr N^f$ with basis $\{\bs n_F, F\in \Delta_{n-1}( T), f\subseteq F\}$, we use Lemma \ref{lm:dualnormal} to find its dual basis. 
For $\bs x_{\alpha} \in \mathcal X_{\mathring{f}}, f \in \Delta_{\ell}(T)$, by Lemma \ref{lem:basisfunctions}, the basis related to $\bs x_{\alpha}$ are
$$
\phi_{\bs\alpha}(\bs x)\bs t_i^f, i=1,\ldots, l; \quad
\frac{1}{\bs n_{f+i,f}\cdot\bs n_{F_i}}\phi_{\bs\alpha}(\bs x)\bs n_{f+i,f},  i\in f^*. 
$$

% For an interpolation points $\bs x\in \mathcal X_{T}$, we collect the $t-n$ basis together and write as $\{\bs e^i_{\bs x}, i=0,\ldots n-1\}$ and its dual basis is $\{\hat{\bs e}^i_{\bs x}, i=0,\ldots n-1\}$. When $\{\bs e^i_{\bs x}, i=0,\ldots n-1\}$ is orthonormal, its dual basis is itself. In general, we can always choose an orthonormal basis for the tangential plane $\mathscr T^f$ but for the normal plane $\mathscr N^f$, we can use Lemma \ref{lm:dualnormal} to find the dual basis. 
...
% It is straightforward to verify the duality
% $$
% N^j_{\bs \beta }(\bs{\phi}_{\bs \alpha}^i) =   \bs{\phi}_{\bs \alpha}^i(\bs x_{\beta}) \cdot \bs e_{\bs x_{\beta}}^j = \delta_{i}^j \delta_{\bs \alpha}^{\bs \beta}, \quad \quad \bs \alpha, \bs \beta\in  \mathbb T^n_k, i,j = 0,\ldots, n-1. 
% $$ 
% \end{proof}
...
The novelty of our approach is that we only need a basis of Lagrange element which is well documented; see Section \ref{sec:lagrangebasis}. Coupling with different $t-n$ decomposition at different sub-simplex, we obtain the classical face elements. 

%\LC{Give a remark to other versions of face element, like Stenberg's element.}
% \LC{Include the global version and show it is div conforming.}
...
V_h^{\div}:=&\{\bs u\in L^2(\Omega;\mathbb R^n): \bs u|_T\in\mathbb P_k(T; \mathbb R^n) \textrm{ for each } T\in\mathcal T_h, \\
 &\textrm{ all the DoFs \eqref{eq:dofvectorndiv} across $F\in\Delta_{n-1}(\mathcal T_h)$ are single-valued}\}.
\end{align*}
By Theorem~\ref{div:equivalentdofs}, $V_h^{\div}\subset H(\div,\Omega)$ and is equivalent to the BDM space.
...
%\LC{Define $n_e$ and $t_e$. Is it local?}
  \begin{enumerate}
...
%\LC{Please correct the lemma below.}
...
%\end{lemma}
%\begin{proof}
%%\LC{Prove it is $H(\div)$-conforming and coincides with BDM space.}
...
\begin{figure}[htp]
  ...
  \centering
  \includegraphics[width=1.5\textwidth]{figures/bdm_dof.png}
  \end{minipage}
  \begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=1.5\textwidth]{figures/bdm_basis.png}
  \end{minipage}
...
%  \label{eq:bdmbasis3d}
%  \bs{\phi}_{\bs \alpha}^i(\bs x) = 
%  \phi_{\bs\alpha}(\bs x) \hat{\bs e}_{\bs x_{\bs \alpha}}^i, 
...
%\end{lemma}
%\begin{proof}
%%\LC{Verify it is $H(\div)$-conforming.}
...
Hence we can identify $\curl\boldsymbol{v}$ as scalar $\rot\boldsymbol{v}$ in two dimensions, and vector $\nabla\times\boldsymbol{v}$ in three dimensions. In general $\curl \bs u$ is a skew-symmetric matrix. 

% \LC{Give examples in 2D and 3D. In 3D, identity it as a vector and in 2D, it is a scalar.}
...
For a vector $\bs v\in \mathbb R^n$ and an $n-1$ dimensional face $F$, the tangential part of $\boldsymbol v$ is
$$
\Pi_F\boldsymbol v := \boldsymbol v|_F - (\boldsymbol v|_F\cdot\boldsymbol n_F)\boldsymbol n_F = \sum_{i=1}^{n-1}(\boldsymbol v|_F\cdot\boldsymbol t_{F,i})\boldsymbol t_{F,i},
$$ 
where $\{ \bs t_{F,i}, i=1,\ldots, n-1 \}$ is an orthonormal basis of $F$. 
As we treat $\curl \bs v$ as a matrix, so is the trace ${\rm tr}_F^{\curl}\boldsymbol v$. The tangential component of $\bs v$ is a vector. Their relation is given in the following lemma.
...
$$
For Lagrange bubble $\mathbb B_k^n(T)$, all components of the vector vanish on $\partial T$ and thus vanish on all sub-simplex with dimension $\leq n-1$. For $\bs u \in \mathbb B_k(\curl, T)$, only the tangential component vanishes, which will imply $\bs u$ vanishes on sub-simplex with dimension less than or equal to $n-2$. 
...
$$
where $\bs n_{F_i, f}$ is a normal vector $f$ sitting on $F_i$. As $\spa \{\bs t_1^f, \ldots, \bs t_{n-2}^f, \bs n_{F_1, f}, \bs n_{F_2, f} \} = \mathbb R^n$, we acquire $\bs u|_f=\bs0$. By the property of face bubbles, we conclude $\bs u$ is a linear combination of $n-1$ face bubbles. 
\end{proof}
...
Notice that the $\curl_f$-bubble function is defined for $\ell \geq 2$ not including edges. Indeed, for an edge $e$ and a vertex ${\bs x}$ of $e$, $\boldsymbol{n}_{e,{\bs x}}$ is $\boldsymbol{t}_{e}$ if $\bs x$ is the ending vertex of $e$ and $-\boldsymbol{t}_{e}$ otherwise. Then for $\ell = 1$
$$
\mathbb B_{k}( e) \bs t_e \oplus \Oplus_{{\bs x}\in \partial e} {\rm span}\{\lambda_{\bs x} \boldsymbol{n}_{e,{\bs x}}\} = \mathbb P_{k}(e)\bs t_e.
$$
which is no longer a bubble function on $e$. 
%Indeed for one dimensional edge $e\in \Delta_1(T)$
%$$
%\mathbb B_k(\curl; e) = \mathbb B_k(\grad; e)\bs t^e = \mathbb B_k(e) \bs t^e.  
%$$

%\LC{Even $\mathbb B_{k}( e)$ can't be called the bubble space as no boundary of a vertex. Vanishing at other vertices is not the definition of bubble space.}
\begin{theorem}
For $k\geq 1$, we have
\begin{align}
...
\end{theorem}
\begin{proof}
%Decomposition \eqref{eq:curlpolynomialdecomp} is a re-arrangement of components in the decomposition \eqref{eq:Pkvecdec}. 
...
$$
In decomposition \eqref{eq:Pkvecdec} shift the normal component one level up:
% and use \eqref{eq:curlfbubble} to get the decomposition \eqref{eq:curlpolynomialdecomp}. 
% Then we write
\begin{align*}
&\Oplus_{\ell = 2}^n\Oplus_{e\in \Delta_{\ell-1}(T)} \big [ \mathbb B_{k}^{\ell-1}(e)\Oplus_{e\subseteq f,f\in \Delta_{\ell}(T)}\mathbb B_k( e)\boldsymbol{n}_{f,e} \big ]\\
=&\Oplus_{e\in\Delta_{1}(T)}\mathbb B_k(e)\bs t_e \oplus \Oplus_{\ell=2}^n\Oplus_{f\in\Delta_{\ell}(T)}\big [\mathbb B_{k}^{\ell}(f) \Oplus_{e\subseteq f,e\in \Delta_{\ell - 1}(T)}\mathbb B_k( e)\boldsymbol{n}_{f,e} \big ],
\end{align*}
where we switch the sum of $e$ and $f$. Then by the characterization \eqref{eq:curlfbubble} of $\mathbb B_k(\curl_f, f)$, we get the decomposition \eqref{eq:curlpolynomialdecomp}.
...
\end{proof}
Decomposition \eqref{eq:curlpolynomialdecomp} is the counterpart of \eqref{eq:Lagdecbubble} for Lagrange elements. In decomposition~\eqref{eq:curlpolynomialdecomp2}, the linear vector polynomial $\mathbb P_1^n(T)$ is redistributed to edges and the second kind N\'ed\'elec element~\cite{Nedelec1986} can be derived. 

\subsection{Tangential-Normal decomposition of second family of N\'ed\'elec element}
For $e\in\Delta_{\ell-1}(T)$, the basis 
...
\int_f \bs u\cdot \bs p \dd s,& \quad \bs p\in \mathbb B_k(\curl_f, f),f\in\Delta_{\ell}(T), \ell=2,\ldots, n-1, \\
...
\end{theorem}
\begin{proof}
The unisolvence of DoFs \eqref{eq:dofveccurl} for $\mathbb P_k^n(T)$ follows from Corollary~\ref{lm:vecLdec} by switching $\{\bs n_1^e, \dots, \bs n_{n-\ell+1}^e\}$ to $\{\bs n_{f,e}, e\subseteq f\in\Delta_{\ell}(T)\}$.
...
%
%
%\LC{The difference is the DoF on edges. Redistribute tangential direction at a vertex to edges. Merge normal DoFs to one.}

For $e\in \Delta_{\ell-1}(T)$, we redistribute the normal DoFs \eqref{eq:dofvectorn} on $e$ to the tangential part \eqref{eq:dofvectorncurl} of $f$ for $e\subseteq f\in\Delta_{\ell}(T)$.

\begin{remark}\rm
 The DoFs of the second kind N\'ed\'elec element in \cite{Nedelec1986,ArnoldFalkWinther2006} are
\begin{align*}
\int_e \bs u\cdot \bs t\ p \dd s,& \quad p\in \mathbb P_{k} (e), e\in \Delta_1(T), \\
\int_f \bs u\cdot \bs p \dd s,& \quad \bs p\in \mathbb P_{k-\ell}^{\ell}(f)+(\Pi_f\bs x)\mathbb P_{k-\ell}(f), f\in\Delta_{\ell}(T), \ell=2,\ldots, n-1, \\
...
V_h^{\curl}:=&\{\bs u\in L^2(\Omega;\mathbb R^n): \bs u|_T\in\mathbb P_k(T; \mathbb R^n) \textrm{ for each } T\in\mathcal T_h, \\
 ...
 &\textrm{ all the DoFs \eqref{eq:dofvectorncurl} across $f\in\Delta_{\ell}(\mathcal T_h)$ are single-valued for  $\ell=1, \ldots, n-1$}\}.
\end{align*}
Since DoFs \eqref{eq:dofveccurl} are equivalent to DoFs \eqref{eq:dofNedelec}, we have $V_h^{\curl}\subset H(\curl,\Omega)$.

%\LC{Can we give a direct proof without resort to Nedelec element to show the tangential component $\Pi_F \bs v$ is continuous?}
\begin{lemma}
We have $V_h^{\curl}\subset H(\curl,\Omega)$.
\end{lemma}
\begin{proof}
For an $F\in\Delta_{n-1}(T)$, DoFs \eqref{eq:dofveccurl} related to $\Pi_F\bs u$ are
\begin{align*}
...
\subsection{A basis for the second-kind N\'ed\'elec element}
...
By Lemma~\ref{lm:dualnormal}, the dual basis of $\{\bs n_{f+i,f}\}_{i\in f^*}$ are $\{\frac{1}{\bs n_{f+i,f}\cdot\bs n_{F_i}}\bs n_{F_i}\}_{i\in f^*}$. For $\bs x_{\alpha} \in \mathcal X_{\mathring{f}}, f \in \Delta_{\ell}(T)$, by Lemma \ref{lem:basisfunctions}, the basis related to $\bs x_{\alpha}$ are
$$
\phi_{\bs\alpha}(\bs x)\bs t_i^f, i=1,\ldots, \ell; \quad
\frac{1}{\bs n_{f+i,f}\cdot\bs n_{F_i}}\phi_{\bs\alpha}(\bs x)\bs n_{F_i},  i\in f^*. 
$$
% \LC{Find the dual basis of $n_{f,e}$. Is it the normal basis in face element?}

Again we present explicit formulae in two and three dimensions. 

\subsubsection{Second-kind N\'ed\'elec element on triangular meshes}
%\begin{definition}
...
\begin{figure}[htp]
\centering
\includegraphics[width=0.8\textwidth]{figures/nedelec.png}
\caption{The left figure shows $\{\bs e_0, \bs e_1\}$ at each interpolation 
point, the right figure shows $\{\hat{ \bs e}_0, \hat{ \bs e}_1\}$ 
at each interpolation point.}
\end{figure}

\subsubsection{Second-kind N\'ed\'elec element on tetrahedron meshes}
%\begin{definition}
...
\begin{figure}[htp]
  \begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=1.5\textwidth]{figures/nedelec_dof.png}
  \end{minipage}
  \begin{minipage}[t]{0.48\textwidth}
  \centering
  \includegraphics[width=1.5\textwidth]{figures/nedelec_basis.png}
  \end{minipage}
  \caption{The left figure shows $\{\bs e_0, \bs e_1, \bs e_2\}$ at each 
      interpolation point, the right figure shows 
      $\{\hat{ \bs e}_0, \hat{ \bs e}_1, \hat{\bs e}_2\}$ 
...
equivalent to providing a indexing rule for global DoFs in the scalar Lagrange
...
\frac{(k-3)(k-2)(k-1)}{6},
\]
are numbers of interpolation points inside edge, face, and cell, respectively. We need an index mapping from $[0: \mc{ldof}-1]$ to  $[0: \mc{gdof}-1]$. See Fig. \ref{fig:assigndofex} for an illustration of the local index and the global index of interpolation points. 
...
%\LC{Try to draw a figure to illustrate the index mapping.}


\subsection{BDM  and second-kind N\'ed\'elec finite element space}
...
$\boldsymbol e'$ which is the dual vector of  $\boldsymbol e$. 
...
according to the dimension of the sub-simplex where the DoFs locate. Note
...
similar with the Lagrange interpolation points. First count the shared DoFs on
...
By the global indexing rule, we also can get a array named
...
$\boldsymbol u \in H(\mathrm{div}, \Omega)$, $p \in L^2(\Omega)$, such that
...
piecewise $k$-degree polynomial space on $\mathcal T_h$ by $V_{k-1}$.  
The corresponding finite element method is: find 
$\boldsymbol u_h \in V_{h,k}^{\div}, p_h \in V_{k-1}$, staify:
\begin{align}
...
method \eqref{exm2eq1} and \eqref{exm2eq2} on uniformly refine mesh with $k = 2, 3, 4$.}
\label{exm2fig1}
\end{figure}

To test the correctness of BDM space we set
$$
\boldsymbol u = 
\begin{pmatrix}
...
= \int_{\Omega}\boldsymbol J \cdot \boldsymbol v_h \ \mathrm d\boldsymbol x, \ 
\boldsymbol v_h \in  \mathring{V}_{h,k}^{\curl}.
\end{align} 

To test the convergence rate of second-kind N\'ed\'elec space, we choose 
\begin{align*}
\omega  &= \tilde \epsilon = \mu = 1,\\
\boldsymbol E &= (f, \sin(x)f, \sin(y)f),\\
 f &= (x^2-x)(y^2-y)(z^2-z),\\
...
\caption{Errors $\|\bs E - \bs E_h\|_{0}$ and 
    ...
    method \eqref{exm1:eq1} on uniformly refine mesh with $k = 2, 3, 4$.}
\label{exm1fig1}
\end{figure}
