...
\textcolor{blue}{In addition to sensitivity analysis, computational efficiency remains a critical challenge for large-scale topology 	optimization, particularly for high-resolution 3D problems.
...
\begin{equation}\label{eq:cmpc}
	...
	\end{aligned}
\end{equation}
where $c(\rho)$ is defined as $\int_{\Omega}\sigma(u):\varepsilon(u)~\mathrm{d}x$, this definition equals twice the strain energy, a convention widely adopted in TO literature, $g(\rho) = v(\rho) - V^*\leq0$ denotes the volume constraint, $v(\rho) = \int_{\Omega}\rho~\mathrm{d}x$ is the material volume, $\varepsilon(u) = \frac{1}{2}(\nabla{u}+\nabla{u}^T)$ is the strain tensor, and $\sigma(u) = D(\rho):\varepsilon(u)$ is the stress tensor. \textcolor{red}{Here $f$ and $t$ denote body forces and surface tractions, respectively. The boundary is decomposed into Dirichlet and Neumann parts such that $\Gamma_D \cup \Gamma_N = \partial\Omega$ and $\Gamma_D \cap \Gamma_N = \emptyset$.}\textcolor{gray}{Here $f$ and $t$ denote body forces and surface tractions, respectively, and $\Gamma_D \cup \Gamma_N = \partial\Omega$.}
...
\textcolor{blue}{\subsection{Additional 2D Benchmark Problems}\label{sec:exp_2d}}
While the MBB beam example presented earlier serves as an excellent benchmark for validating the performance and stability of optimization algorithms, SOPTX is designed as a general-purpose framework capable of flexibly handling diverse boundary conditions and loading scenarios. To further demonstrate the applicability of SOPTX across a broader range of 2D engineering contexts, this section introduces two additional classic benchmark problems: the bridge structure and the half-wheel structure. Numerical experiments demonstrate that users can seamlessly apply the framework to new physical problems by simply inheriting and defining new PDE data classes, without the need to modify the core solver or optimization modules. This capability fully highlights the advantages of SOPTX's decoupled architecture.

2D Simple Bridge Structure

Consider the bridge structure design problem shown in Figure [Ref]. In this example, the design domain is defined as a $60 \times 30$ rectangular region, discretized by a uniform quadrilateral mesh. The boundary conditions consist of fully fixed supports at the two bottom corners. A downward concentrated load $T=-1$ is applied at the midpoint of the bottom edge. The target volume fraction is set to 0.3, and the sensitivity filter radius is set to $r=2.4$ (chosen to match the mesh element size, ensuring structural smoothness and suppressing checkerboard patterns).
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\textwidth]{figures/simple_bridge_2d.png}
