...
\end{figure}
As seen in the figure, the two methods yield noticeably different results. 

% The modular architecture of the SOPTX framework allows seamless switching between different filtering strategies. Users can simply replace the filter class to apply a different method without modifying the rest of the code. This section takes the MBB beam example from Section~\ref{sec:exp_mbb_beam} as a test case to demonstrate the flexibility of SOPTX in applying various filters and to compare the resulting topologies. All simulation parameters are kept the same as in Section~\ref{sec:exp_mbb_beam}, using a $150 \times 50$ uniform quadrilateral mesh. Unless otherwise stated, the filter radius is set to $6$.
