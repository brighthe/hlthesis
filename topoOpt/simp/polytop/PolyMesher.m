...
while (It<MaxIter && Err>Tol)
    ...
    Area = sum(abs(A));
    Err = sqrt(sum((A-A(1)).^2)/NElem)/Area^1.5;
...
[Node,Element] = PolyMshr_ClpHsEdgs(Node,Element,0.1); % Remove small edges
[Node,Element] = PolyMshr_RsqNdGs(Node,Element,NElem); % Reorder Nodes
BC=Domain('BC',Node); Supp=BC{1}; Load=BC{2};
...
R_P = repmat(P,[nBdrySegs,1]);
R_P(I(:,1),:) = P(I(:,1),:) - 2*n1(I(:,1)).*d(I(:,1));
R_P(I(:,2),:) = P(I(:,2),:) - 2*n2(I(:,2)).*d(I(:,2));
J = abs(d_R_P(:,end))>eta*abs(d(:,1)) & d_R_P(:,end)>0;
...
Node = Node0(cNode,map); 
[Node,Element] = PolyMshr_RbldLists(Node0,Element0(1:NElem),cNode);
% ------------------------------------------------------------------------ %
function [Node0,Element0] = PolyMshr_ClpHsEdgs(Node0,Element0,Tol)
while(true)
    cEdge = [];
