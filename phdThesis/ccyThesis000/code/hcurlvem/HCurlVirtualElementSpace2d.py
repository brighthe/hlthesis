...
from fealpy.quadrature import PolygonMeshIntegralAlg, TriangleQuadrature
...
class HCurlVirtualElementSpace2d():
    ...
    def __init__(self, mesh):
        ...
        self.cellbarycenter = mesh.cell_barycenter()
    ...
    def curl_val(self, c2e, cell2dofLoc):
        """
        @brief 返回每个单元上每个基函数的旋度值
        """
        mesh = self.mesh
        cellmea = self.cellmeasure
        el = self.mesh.entity_measure('edge')
        cell2edgeSign = (mesh.ds.cell_to_edge_sign().astype(np.float_)-0.5)*2
        c2eS      = np.split(cell2edgeSign*el[c2e], cell2dofLoc[1:-1])

        f = lambda x : x[0]/x[1] 
        val = list(map(f, zip(c2eS, cellmea)))
        return val
    ...
    def curl_matrix(self, beta=None, dtype=np.float_):
        ...
        c2eS      = np.split(cell2edgeSign*el[cell2edge], cell2edgeLoc[1:-1])
    ...
    def matrix_PI(self):
        ...
        edgeint = edgeint*edgemea[:, None]

        cell2edge, cell2edgeLoc = mesh.ds.cell_to_edge()

        # c2eint[:, 0] 每个单元每条边函数 (x1 - barx1) 的积分
        # c2eint[:, 1] 每个单元每条边函数 (barx0 - x0) 的积分
        c2eint = np.zeros((2, cell2edgeLoc[-1]), dtype=np.float_)
        c2eint[0, cell2edgeLoc[edge2cell[:, 1]]+edge2cell[:, 3]] = edgeint[:, 1]
        c2eint[0, cell2edgeLoc[edge2cell[:, 0]]+edge2cell[:, 2]] = edgeint[:, 0]
        c2eint[1, cell2edgeLoc[edge2cell[:, 1]]+edge2cell[:, 3]] = edgeint[:, 3]
        c2eint[1, cell2edgeLoc[edge2cell[:, 0]]+edge2cell[:, 2]] = edgeint[:, 2]
        PI = np.hsplit(c2eint, cell2edgeLoc[1:-1])
        for i in range(NC):
            PI[i] /= -cellmea[i]
        return PI
    ...
    def mass_matrix(self, alpha=None, dtype=np.float_):
        ...
        cellbar = self.cellbarycenter 
        cellmea = self.cellmeasure
        edgetan = mesh.edge_unit_tangent()
        alphaK = np.ones(NC, dtype=np.float_) if alpha is None else alpha(cellbar).reshape(-1)
        PI = self.PI
        ...
        idx = 0
        NV = cell2edgeLoc[1:]-cell2edgeLoc[:-1]
        M = np.zeros((np.sum(NV**2), ), dtype=dtype)
        for i in range(NC):
            N = -tN[i]@PI[i]
            N[np.diag_indices_from(N)] += 1
            val = alphaK[i]*cellmea[i]*(PI[i].T@PI[i]) + cellmea[i]*(N.T@N)
            M[idx:idx+N.shape[0]**2] = val.reshape(-1) 
            idx += N.shape[0]**2

        f1 = lambda x: np.repeat(x, x.shape[0])
        f2 = lambda x: np.tile(x, x.shape[0])

        I = np.concatenate(list(map(f1, c2d)))
        J = np.concatenate(list(map(f2, c2d)))
    ...
    def projection_to_smspace(self, uh):
        ...
        f = lambda x : x[0]@x[1]
        uhK = np.concatenate(list(map(f, zip(PI, uhK)))).reshape(-1, 2)
        return uhK
    ...
    def curl_error(self, cu, uh, celltype=False):
        ...
        cell2dof, cell2dofLoc = self.dof.cell_to_dof()
        uhK = np.split(uh[cell2dof], cell2dofLoc[1:-1])
        val = self.curl_val(cell2dof, cell2dofLoc)
    ...
    def source_vector(self, f, dtype=np.float_):
        ...
        cell2dof, c2dloc = self.dof.cell_to_dof()
        ...
        FK = np.zeros_like(cell2dof, dtype=dtype)
        for i in range(NC):
            FK[c2dloc[i]:c2dloc[i+1]] = (F_[i, None, :]@self.PI[i]).reshape(-1)
