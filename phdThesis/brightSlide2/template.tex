...
\begin{document}
	...
	\begin{frame}
		...
		\begin{tcolorbox}[
			...
			top=3pt, bottom=3pt, left=8pt, right=8pt]
			{\fontsize{8}{12}\selectfont\sffamily
				在获得灵敏度信息后，需基于梯度的优化算法驱动设计变量迭代更新；
				OC 法 与 MMA 是工程中应用最广的两类代表算法。}
		\end{tcolorbox}
		...
		\begin{columns}[T, totalwidth=\linewidth]
			...
			\begin{column}{0.485\linewidth}
				\begin{tcolorbox}[
					...
					colbacktitle=structure.fg, coltitle=white,
					fonttitle=\fontsize{8}{10}\selectfont\bfseries\sffamily,
					title={优化准则法 OC (Optimality Criteria)},
					arc=3pt, boxrule=0.5pt,
					toptitle=3pt, bottomtitle=3pt,
					top=5pt, bottom=5pt, left=6pt, right=6pt,
					equal height group=cards-02-6, valign=top]
					{\fontsize{7.5}{11}\selectfont\sffamily
						\setbeamertemplate{itemize item}{\raisebox{1pt}{\tiny$\bullet$}}
						\setlength{\leftmargini}{0.85em}
						
						\textbf{\circnum{01}\enspace 基本思想}\par
						\vspace{2pt}
						\begin{itemize}
							\item 由 KKT 最优性条件推导设计变量的启发式显式更新格式
						\end{itemize}
						
						\textbf{\circnum{02}\enspace 经典更新格式 (Bendsøe)}\par
						\vspace{2pt}
						\begin{equation*}
							\rho_i^{(k+1)} = \rho_i^{(k)} \bigl( B_i^{(k)} \bigr)^\eta
						\end{equation*}
						\vspace{-10pt}
						\begin{itemize}
							\item $B_i$：由灵敏度比值确定的局部更新系数（KKT 驻点条件）
							\item $\eta$：阻尼系数，配合移动限制 $m$ 控制单步更新幅度
						\end{itemize}
						
						\textbf{\circnum{03}\enspace 适用与局限}\par
						\vspace{2pt}
						\begin{itemize}
							\item \textbf{适用}：单约束、自伴随问题（如柔顺度 + 体积约束）
							\item \textbf{局限}：多约束下丧失显式更新优势；非自伴随问题易诱发震荡
						\end{itemize}
					}
				\end{tcolorbox}
			\end{column}
			...
			\begin{column}{0.485\linewidth}
				\begin{tcolorbox}[
					...
					colbacktitle=structure.fg, coltitle=white,
					fonttitle=\fontsize{8}{10}\selectfont\bfseries\sffamily,
					title={移动渐近线方法 MMA (Method of Moving Asymptotes)},
					arc=3pt, boxrule=0.5pt,
					toptitle=3pt, bottomtitle=3pt,
					top=5pt, bottom=5pt, left=6pt, right=6pt,
					equal height group=cards-02-6, valign=top]
					{\fontsize{7.5}{11}\selectfont\sffamily
						...
						\textbf{\circnum{01}\enspace 基本思想 (Svanberg)}\par
						\vspace{2pt}
						\begin{itemize}
							\item 序列凸规划 (SCP) 的经典代表
							\item 用动态更新的移动渐近线构造可分严格凸近似
						\end{itemize}
						
						\textbf{\circnum{02}\enspace 严格凸近似子问题}\par
						\vspace{2pt}
						\begin{equation*}
							\tilde{f}_i^{(k)}(\boldsymbol{\rho}) = \sum_{j=1}^{N_d} \!\left( \frac{p_{ij}^{(k)}}{U_j^{(k)} - \rho_j} + \frac{q_{ij}^{(k)}}{\rho_j - L_j^{(k)}} \right) + r_i^{(k)}
						\end{equation*}
						\vspace{-10pt}
						\begin{itemize}
							\item $L_j^{(k)}, U_j^{(k)}$：自适应更新的下/上移动渐近线
						\end{itemize}
						
						\textbf{\circnum{03}\enspace 适用与优势}\par
						\vspace{2pt}
						\begin{itemize}
							\item \textbf{适用}：多约束、高度非线性的复杂拓扑优化问题
							\item \textbf{优势}：子问题严格凸，可用原对偶内点法高效求解
						\end{itemize}
					}
				\end{tcolorbox}
			\end{column}
		...
		\begin{tcolorbox}[
			...
			\centering
			{\fontsize{8}{11}\selectfont\sffamily\color{white}
				OC 法实现简单、计算高效；MMA 通用性强、可处理多约束复杂问题}
		\end{tcolorbox}
	...
	\begin{frame}
		...
		\begin{tcolorbox}[
			...
			top=3pt, bottom=3pt, left=8pt, right=8pt]
			{\fontsize{8}{12}\selectfont\sffamily
				连续体拓扑优化无正则化时数学上不适定，易出现网格依赖与棋盘格现象；
				过滤与投影是控制最小长度尺度、获得网格无关设计的主流框架。}
		\end{tcolorbox}
		...
		\begin{columns}[T, totalwidth=\linewidth]
			...
			\begin{column}{0.325\linewidth}
				\begin{tcolorbox}[
					...
					colbacktitle=structure.fg, coltitle=white,
					fonttitle=\fontsize{8}{10}\selectfont\bfseries\sffamily,
					title={\circnum{01}\enspace 灵敏度过滤 (Sigmund)},
					arc=3pt, boxrule=0.5pt,
					toptitle=0.25pt, bottomtitle=0.25pt,
					top=3pt, bottom=3pt, left=6pt, right=6pt,
					equal height group=cards-02-7]
					{\fontsize{7.5}{11}\selectfont\sffamily
						\setbeamertemplate{itemize item}{\raisebox{1pt}{\tiny$\bullet$}}
						\setlength{\leftmargini}{0.85em}
						\begin{itemize}
							...
							\setlength{\parsep}{0pt}
							\setlength{\parskip}{0pt}
							\item 启发式方法，作用于灵敏度（梯度）场而非设计变量本身
							\item 通过卷积平滑梯度场，有效抑制棋盘格现象
							\item 形式简单、计算高效，工程应用最广泛
							\item 但缺乏严格变分基础，不直接约束设计空间
						\end{itemize}
					}
				\end{tcolorbox}
			\end{column}
			...
			\begin{column}{0.325\linewidth}
				\begin{tcolorbox}[
					...
					colbacktitle=structure.fg, coltitle=white,
					fonttitle=\fontsize{8}{10}\selectfont\bfseries\sffamily,
					title={\circnum{02}\enspace 密度过滤 (Bruns \& Tortorelli)},
					arc=3pt, boxrule=0.5pt,
					toptitle=0.25pt, bottomtitle=0.25pt,
					top=3pt, bottom=3pt, left=6pt, right=6pt,
					equal height group=cards-02-7]
					{\fontsize{7.5}{11}\selectfont\sffamily
						...
						\begin{itemize}
							...
							\setlength{\parsep}{0pt}
							\setlength{\parskip}{0pt}
							\item 解耦“数学设计空间”与“物理材料空间”
							\item 物理密度场为设计变量的卷积平滑：
							\vspace{-2pt}
							\begin{equation*}
							...
							\end{equation*}
							\vspace{-6pt}
							\item 具有严格的数学适定性保证 (Bourdin)
							\item 但边界存在中间密度过渡（灰度带）
						\end{itemize}
					}
				\end{tcolorbox}
			\end{column}
			...
			\begin{column}{0.325\linewidth}
				\begin{tcolorbox}[
					...
					colbacktitle=structure.fg, coltitle=white,
					fonttitle=\fontsize{8}{10}\selectfont\bfseries\sffamily,
					title={\circnum{03}\enspace 投影方法 (Guest, Sigmund)},
					arc=3pt, boxrule=0.5pt,
					toptitle=0.25pt, bottomtitle=0.25pt,
					top=3pt, bottom=3pt, left=6pt, right=6pt,
					equal height group=cards-02-7]
					{\fontsize{7.5}{11}\selectfont\sffamily
						...
						\begin{itemize}
							...
							\setlength{\parsep}{0pt}
							\setlength{\parskip}{0pt}
							\item 在密度过滤后引入非线性投影，锐化边界灰度
							\item 形成“三场表述”：$\rho \to \tilde{\rho} \to \bar{\rho}$
							\item 双曲正切型投影（最常用）：
							\vspace{-2pt}
							\begin{equation*}
					...
					\begin{columns}[T, totalwidth=\linewidth]
						...
						\begin{column}{0.42\linewidth}
							...
							\begin{itemize}
								...
								\item 乘子 \(\lambda^{(k)}_{e,i}\) 与罚参数 \(\mu^{(k)}\) 分别刻画线性惩罚和二次惩罚贡献。
							\end{itemize}
						\end{column}
		...
		\only<1>{
			\begin{tcolorbox}[
				...
				{\fontsize{7.55}{10.4}\selectfont\sffamily
					...
					\Longrightarrow
					\boldsymbol{\sigma}_h\boldsymbol{n}\text{ 单值}
					\Longrightarrow
					\boldsymbol{\Sigma}_h^k
					\subset
					H(\operatorname{div};\Omega;\mathbb{S}).
