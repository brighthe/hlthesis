...
for i in range(nt):
    ...
    phi0[:] = lsfemsolver.mumps_solve(phi0 = phi0, dt = dt, u = u)

    # Save the current state if output is enabled
    lssolver.output(phi = phi0, u = u, timestep = i+1, output_dir = output, filename_prefix = 'exp1_lsf_without_reinit_mumps')
