...
for i in range(nt):
    ...
    u1 = space1.interpolation(velocity_field_at_i, dim=2)
    ...
    lssolver.output(phi = phi0, u = u, timestep = i+1, output_dir = output, filename_prefix = 'new_u_t_lsf_without_reinit_mumps')
    lssolver.output(phi = s0, u = u, timestep = i+1, output_dir = output, filename_prefix = 'old_u_t_lsf_without_reinit_mumps')
